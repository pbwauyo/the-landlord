...
class AddRental : Fragment() {
    ...
    private fun initPickers(){
        ...
        monthYearPicker.setOnDateSetListener {
                year, monthOfYear ->
            rentalViewModel.setRentStartMonth(monthOfYear.toString().padStart(2, '0')) //add trailing zero
            rentalViewModel.setRentStartYear(year.toString())

            Log.d(TAG, "$monthOfYear")
        }
    }
    ...
    private fun initTenantFieldsListeners(){
        binding!!.tenantContactCardView.setOnClickListener {
            if (!it.isActivated){
                Toast.makeText(activity, EMPTY_TENANT_NAME_ERROR, Toast.LENGTH_LONG).show()
            }
        }
